...
export default async function DoctorMonitorPage() {
  await requireRole(["doctor"]);

  const supabase = await createClient();
  const { data, error } = await supabase.rpc("get_doctor_monitor_dashboard");
        ...
        <div className="flex flex-col gap-4 lg:flex-row lg:items-center lg:justify-between">
          <div>
            ...
            <p className="mt-3 max-w-3xl text-neutral-600">
              Theo dõi tuân thủ uống thuốc, chỉ số sức khỏe và cảnh báo của các hồ sơ đã được phân công.
            </p>
          </div>
        ...
        <div className="rounded-3xl border border-neutral-200 bg-white p-6 shadow-sm lg:col-span-1">
          ...
          <div className="mt-5 space-y-4">
            {dashboard.profiles.length === 0 ? (
              <p className="text-sm text-neutral-500">Chưa có bệnh nhân được phân công.</p>
            ) : (
              dashboard.profiles.map((profile) => (
                <article key={profile.id} className="rounded-3xl border border-neutral-200 p-4">
                  ...
                  <p className="mt-1 text-sm text-neutral-600">
                    Dị ứng: {profile.allergies ?? "Chưa nhập"}
                  </p>
                </article>
              ))
            )}
